...
def plot_transition_probabilities(ax, world, border=None, **kwargs):
	...
	Args:
		...
		`pyplot.tripcolor`.
	"""
	xy = [(x - 0.5, y - 0.5) for y, x in product(range(size + 1), range(size + 1))]
	xy += [(x, y) for y, x in product(range(size), range(size))]

	t, v = [], []
	for sy, sx in product(range(size), range(size)):
		...
		# compute cell points
		bl, br = sy * (size + 1) + sx, sy * (size + 1) + sx + 1
		tl, tr = (sy + 1) * (size + 1) + sx, (sy + 1) * (size + 1) + sx + 1
		cc = (size + 1)**2 + sy * size + sx
...
def plot_state_values(ax, world, values, border, **kwargs):
	...
	if border is not None:
		if is_square(world.n_states):
			...
			for i in range(0, size + 1):
				ax.set_yticks([])
				ax.plot([-0.5, size - 0.5], [1 - 0.5, 1 - 0.5], **border, label=None)
				for x in range(size):
...
def plot_stochastic_policy(ax, env, policy, border=None, **kwargs):
	...
	if is_square(env.observation_space.n):
		...
		for sy, sx in product(range(size), range(size)):
			# print(sy, sx)
			state = (max(0, min(sy,size-1))*size + max(0, min(size-1, sx)))
		...
		xy = [(x - 0.5, 1 - 0.5) for x in range(size + 1)]
		# print(xy)
		xy += [(x, 1) for x in range(size)]
		# print(xy)
		...
		for sx in range(size):
			...
			# compute cell points
			bl, br = 1 * (size + 1) + sx, 1 * (size + 1) + sx + 1
			tl, tr = (1 + 1) * (size + 1) + sx, (1 + 1) * (size + 1) + sx + 1
			cc = (size + 1)**2 + 1 * size + sx
