...
# Create Data Frame
df = pd.DataFrame(columns=["Number of Trajectories", "EVD VAIRL", "EVD IAVI", "EVD IQL", "EVD MAXENT", "PR VAIRL", "PR IAVI", "PR True IAVI", "PR Maxent", "PR IQL", "PR True IQL", "Runtime VAIRL", "Runtime IAVI", "Runtime IQL", "Runtime Maxent"]) 
...
for run in range(1,5):
	...
	for t in range(len(num_t)):
		...
		# Calculate policy based on this
		...
		# Time Check
		t_vairl = end-start + a_p_time
		# Correlation check
		pr_vairl = np.corrcoef(v_true, v_learned)[0,1]
		#######################################################################################################
		# IQL Loop
		start = time.time()
		...
		# Maxent Loop
		start = time.time()
		r = maxent.irl(env, GAMMA, ts, 0.01, epochs=100)
		end = time.time()
		t_maxent = end-start
		...
		t_vairl = end-start + a_p_time
		# Correlation check
		pr_maxent = np.corrcoef(v_true, v_maxent)[0,1]

		df.loc[ np.int(t+run*len(num_t)) ] = [ num_t[t], np.square(v_true - v_learned).mean() , np.square(v_true - v_iavi).mean(), np.square(v_true - v_iql).mean(), np.square(v_true - v_learned).mean(), \
																	pr_vairl, pr_iavi, pr_q_iavi, pr_iql, pr_q_iql , pr_maxent, \
																	t_vairl, t_iavi, t_iql, t_maxent ]
		# Plot graphs
		...
		plt.savefig(os.path.abspath(os.getcwd())+"/img/ow_img/{}/{}/quiver_div_value_after_{}_trajectories_run_{}.png".format( run,epochs, len(ts), run))
		plt.clf()
