...
from gym_objectworld.solvers import maxent_irl
import time
import pandas as pd
import pickle
...
for t in range(len(num_t)):
	...
	trajectories = ts[:1024]

	print(len(trajectories))

	start = time.time()
	r, policy = maxent_irl.irl(env, ts, 0.01, gamma, p_transition)
	end = time.time()
	t_maxent = end-start
	# Calculate value based on this reward
	maxent_val, _ = V.value_iteration(env, r, gamma)
	# Compare
	v_maxent = V.policy_eval(policy, ground_r, env, np.int(size**2), 5)
	t_vairl = end-start
	# Correlation check
